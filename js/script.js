...
function initMusicToggle(){
  ...
  btn.addEventListener('click', function(e){
    e.preventDefault();
    if(!audio.src){
      console.warn('Audio source kosong');
      return;
    }
    if(!playing){
      audio.play().then(()=>{
...
function initCountdowns(){
  ...
  els.forEach(el=>{
    const iso = el.dataset.date ||
      (el.id==='countdown1' ? '2025-12-21T08:00:00+07:00' :
      el.id==='countdown2' ? '2025-12-28T09:00:00+07:00' : null);
  ...
  if(navigator.clipboard){
    navigator.clipboard.writeText(text).then(()=>{
      ...
      if(btn){
        ...
        setTimeout(()=> btn.textContent='Copy Nomor',2000);
      }
...
function initParticles(){
  ...
  function tick(){
    ...
    for(let i=particles.length-1;i>=0;i--){
      ...
      g.addColorStop(0,`rgba(255,240,180,${0.95*p.alpha})`);
      g.addColorStop(0.5,`rgba(255,200,90,${0.5*p.alpha})`);
      g.addColorStop(1,`rgba(255,180,70,${0.03*p.alpha})`);
      ctx.fillStyle=g;
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  if(guest){
    const g=document.getElementById("guest-name");
